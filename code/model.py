...
from transformers import XLMRobertaModel, BertConfig, XLMRobertaConfig, BertTokenizer, XLMRobertaTokenizer
...
class MyBertmodel(BertPreTrainedModel):
  def __init__(self, config, lambda_boundary=0, event_embedding_size=200, emb_size=768, group_size=64):
    ...
    if event_embedding_size > 0:
      ...
      self.classifier = nn.Sequential(nn.Linear(config.hidden_size * 5 + config.len_dim, config.hidden_size),
        activation_func, nn.Dropout(config.hidden_dropout_prob), nn.Linear(config.hidden_size, self.num_labels))
    else:
      self.event_embedding = None
      self.classifier = nn.Sequential(nn.Linear(config.hidden_size * 4 + config.len_dim, config.hidden_size),
        activation_func, nn.Dropout(config.hidden_dropout_prob), nn.Linear(config.hidden_size, self.num_labels))
  ...
  def encode(self, input_ids, token_type_ids, attention_mask, head_mask, inputs_embeds, position_ids,
             output_hidden_states, return_dict):
    start_tokens = [101]
    end_tokens = [102]
    sequence_output, attention = process_long_input(self.bert, input_ids, token_type_ids, attention_mask, start_tokens,
                                                    end_tokens, head_mask, inputs_embeds, position_ids,
                                                    output_hidden_states, return_dict)
  ...
  def context_pooling(self, value_matrix, trigger_att,
                      hidden_rep):  # 文章中RLIG 和STCP的核心实现函数，基于value_matrix和trigger的注意力头得到对上下文和角色信息的关注度
  ...
  def forward(self, input_ids=None, attention_mask=None, labels=None, spans=None, span_lens=None, label_masks=None,
    trigger_index=None, info_dicts=None, start_labels=None, end_labels=None, token_type_ids=None, position_ids=None,
    head_mask=None, inputs_embeds=None, output_hidden_states=None, return_dict=None, ):

    last_hidden_state, attention = self.encode(input_ids, token_type_ids, attention_mask, head_mask, inputs_embeds,
                                               position_ids, output_hidden_states, return_dict)
    ...
    if self.event_embedding is not None:
      logits = torch.cat((
        span_feature, trigger_feature, torch.abs(span_feature - trigger_feature), span_feature * trigger_feature,
    ...
    if self.lambda_boundary > 0:
      ...
      if start_labels is not None and end_labels is not None:
        ...
        loss += self.lambda_boundary * (
            loss_fct(start_logits.view(-1, 2), start_labels.contiguous().view(-1)) + loss_fct(end_logits.view(-1, 2),
                                                                                              end_labels.contiguous().view(
                                                                                                -1)))
    # loss += ident_loss
    return {'loss': loss, 'logits': logits, 'spans': spans, }
...
class MyRobertamodel(RobertaPreTrainedModel):
  def __init__(self, config, lambda_boundary=0, event_embedding_size=200, emb_size=1024, group_size=64):
    super().__init__(config)
    self.num_labels = config.num_labels
    self.roberta = XLMRobertaModel.from_pretrained("xlm-roberta-base", config=config)
    self.emb_size = emb_size
    self.group_size = group_size
    self.dropout = nn.Dropout(config.hidden_dropout_prob)
    activation_func = nn.ReLU()
  ...
  def encode(self, input_ids, token_type_ids, attention_mask, head_mask, inputs_embeds, position_ids,
    ...
    start_tokens = [0]
    end_tokens = [2, 2]
    sequence_output, attention = process_long_input(self.roberta, input_ids, token_type_ids, attention_mask,
                                                    start_tokens, end_tokens, head_mask, inputs_embeds, position_ids,
                                                    output_hidden_states, return_dict)
    return sequence_output, attention
